...
const AESGCM_CEK_INFO_PREFIX = new TextEncoder().encode("Content-Encoding: aesgcm\0");
const AESGCM_NONCE_INFO_PREFIX = new TextEncoder().encode("Content-Encoding: nonce\0");
...
export function subscriberFromRaw(
  ...
  if (uaPrivate.length !== PRIVATE_KEY_LEN) {
    throw new Error(`uaPrivate must be ${PRIVATE_KEY_LEN} bytes, got ${uaPrivate.length}`);
  }
  if (uaPublic.length !== SEC1_UNCOMPRESSED_LEN || uaPublic[0] !== 0x04) {
    throw new Error("uaPublic must be 65-byte uncompressed SEC1 (leading 0x04)");
  }
  if (authSecret.length !== AUTH_SECRET_LEN) {
    throw new Error(`authSecret must be ${AUTH_SECRET_LEN} bytes, got ${authSecret.length}`);
  }
...
export function decryptAes128gcm(raw: Uint8Array, sub: Subscriber): Uint8Array {
  ...
  const _rs = new DataView(raw.buffer, raw.byteOffset + SALT_LEN, RS_LEN).getUint32(0, false);
  const idlen = raw[SALT_LEN + RS_LEN]!;
  if (idlen !== KEYID_LEN) {
    throw new Error(`unsupported keyid length: ${idlen} (expected ${KEYID_LEN})`);
  }
...
export function decryptAesgcm(
  ...
  const PREFIX_LEN = 6;
  const context = new Uint8Array(PREFIX_LEN + 2 + SEC1_UNCOMPRESSED_LEN + 2 + SEC1_UNCOMPRESSED_LEN);
  context.set(new TextEncoder().encode("P-256\0"), 0);
  new DataView(context.buffer).setUint16(PREFIX_LEN, SEC1_UNCOMPRESSED_LEN, false);
  ...
  if (2 + padLen > plaintextPadded.length) throw new Error("aesgcm padding length out of range");
...
function hkdf(salt: Uint8Array, ikm: Uint8Array, info: Uint8Array, len: number): Uint8Array {
  // Node's hkdfSync returns an ArrayBuffer.
  const out = hkdfSync("sha256", Buffer.from(ikm), Buffer.from(salt), Buffer.from(info), len);
  return new Uint8Array(out);
}

function aesGcmDecrypt(key: Uint8Array, nonce: Uint8Array, body: Uint8Array): Uint8Array {
...
function stripRfc8188Padding(plain: Uint8Array): Uint8Array {
  ...
  if (plain[lastNonzero] !== 0x02) throw new Error("unexpected RFC 8188 delimiter");
